...
public class HadoopPigAmbrose extends JavaProcessJob {
	...
	public static String PIG_AMBROSE_MAIN = "azkaban.jobtype.PigAmbroseMain";
	...
	private File pigLogFile;
	
	private static final String HADOOP_SECURITY_MANAGER_CLASS_PARAM = "hadoop.security.manager.class";
	
	public HadoopPigAmbrose(String jobid, Props sysProps, Props jobProps, Logger log) throws IOException {
		super(jobid, sysProps, jobProps, log);
		
		shouldProxy = getSysProps().getBoolean("azkaban.should.proxy", false);
		
		recordMetaData = getJobProps().getBoolean("azkaban.record.metadata", false);
		
		jobProps.put("env."+"azkaban.should.proxy", String.valueOf(shouldProxy));
	...
	public void run() throws Exception {
		...
		File tokenFile = null;
		File outputMetaData = null;
		if(shouldProxy) {
			userToProxy = getJobProps().getString("user.to.proxy");
			getLog().info("Need to proxy. Getting tokens.");
			// get tokens in to a file, and put the location in props
			tokenFile = getHadoopTokens(getJobProps());
		}
		if(recordMetaData) {
			//outputMetaData = File.createTempFile(getId()+"_", "_metadata", new File(getWorkingDirectory()));
			if(getJobProps().containsKey("azkaban.job.metadatafile")) {
				outputMetaData = new File(getJobProps().get(""));
				outputMetaData.createNewFile();
				getJobProps().put("env."+"OUTPUT_METADATA_FILE", outputMetaData.getAbsolutePath());
			}
			else {
				getLog().error("metadata file not set. not recording job metadata");
			}		
		}
		try {
			super.run();
	...
	private HadoopSecurityManager loadHadoopSecurityManager(Props props, Logger logger) throws RuntimeException {
		...
		Class<?> hadoopSecurityManagerClass = props.getClass(HADOOP_SECURITY_MANAGER_CLASS_PARAM, true, HadoopPigAmbrose.class.getClassLoader());
