...
public class HadoopHiveJob extends JavaProcessJob {
  ...
  private HadoopSecurityManager loadHadoopSecurityManager(Props props, Logger logger) throws RuntimeException {
    ...
    Class<?> hadoopSecurityManagerClass =
        props.getClass(HADOOP_SECURITY_MANAGER_CLASS_PARAM, true, HadoopHiveJob.class.getClassLoader());
    ...
    try {
      ...
      getLog().error(
          "Could not instantiate Hadoop Security Manager " + hadoopSecurityManagerClass.getName() + e.getCause());
  ...
  protected String getMainArguments() {
    ...
    if (map != null) {
      for (Map.Entry<String, String> entry : map.entrySet()) {
        list.add("-hiveconf "
            + StringUtils.shellQuote(entry.getKey() + "=" + entry.getValue(), StringUtils.SINGLE_QUOTE));
      }
    }
