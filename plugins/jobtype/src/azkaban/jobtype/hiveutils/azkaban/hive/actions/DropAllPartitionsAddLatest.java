...
/**
 * Drop all the existing partitions in specified table and then add the latest/greatest/highest
 * directory in the specified subdirectory and add that as the only partition in the table.  This
 * action is suitable for tables with full replacement policies where we only want one partition.</p>
 *
 * For example, if we have a table foo with derived_date partitions that correspond to directories:</p>
 *   /foo/2012-01-01</p>
 *       /2012-01-02</p>
 *       /2012-01-03</p>
 * And currently only have derived_date="2012-01-02" registered, we want to drop that partition and
 * add "2012-01-03" as the only partition.</p>
 * </p>
 * This action makes several assumptions:
 * <ul>
 *   <li>The databases are named the same as the directories in tableLocations, ie /derived/member corresponds to some database
 *       with a table named member</li>
 *   <li>The table is has only a single partition column</li>
 *   <li>The partition name corresponds to directories within the specified tableLocations</li>
 *   <li>The partition values are the same name as the directories within the specified tableLocations</li>
 *   <li>The partition directories are named in ascending order such that a sort will give us the one to add and
 *   we will drop all but that one</li>
 * </ul>
 */
@AzkHiveAction(DROP_ALL_PARTITIONS_AND_ADD_LATEST)
public class DropAllPartitionsAddLatest implements HiveAction {
  private final static Logger LOG = Logger.getLogger("com.linkedin.hive.azkaban.hive.actions.DropAllPartitionsAddLatest");
  ...
  public DropAllPartitionsAddLatest(Properties p, HiveQueryExecutor hqe) throws HiveViaAzkabanException {
  ...
  private ArrayList<HQL> addAndDrop(FileSystem fs, String basepath, String table) throws IOException, HiveViaAzkabanException {
    ...
    if(directories.size() == 0) {
      throw new HiveViaAzkabanException("No directories to remove or add found in " + tableLocations);
    }
    ...
    LOG.info("For table " + table + ", going to add " + toAdd + " and attempt to drop " + directories.size() + " others");
    for (String directory : directories) {
      toDropAndAdd.add(new DropPartitionHQL(table, partition, directory, true));
    }

    toDropAndAdd.add(new AddExternalPartitionHQL(table, partition, toAdd, toAdd, true));
    return toDropAndAdd;
  }
