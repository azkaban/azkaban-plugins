...
import static azkaban.jobtype.hiveutils.azkaban.hive.actions.DropAllPartitionsAddLatest.*;
 ...
 * file.  The query can be specified via:
 *   * hive.query = a single-line query that will be fed to Hive
 *   * hive.query.nn = a two-digit padded series of lines that will be joined
 *                     and fed to to Hive as one big query
 */
public class HiveViaAzkaban {
  ...
  public void run() throws HiveViaAzkabanException {
    ...
    if(!p.containsKey(AZK_HIVE_ACTION)) {
        throw new HiveViaAzkabanException("Must specify a " + AZK_HIVE_ACTION + " key and value.");
    }
    ...
    } else if(hive_action.equals(DROP_AND_ADD)) {
      action = new DropAllPartitionsAddLatest(p, hqe);
    } else if(hive_action.equals(UpdateTableLocationToLatest.UPDATE_TABLE_LOCATION_TO_LATEST)) {
      action = new UpdateTableLocationToLatest(p, hqe);
    } else {
      throw new HiveViaAzkabanException("Unknown value (" + hive_action + ") for value " + AZK_HIVE_ACTION);
    }
