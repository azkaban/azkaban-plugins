...
public class ReportalUtilTest
{
  ...
  public void testSortExecutableNodesNullFlow()
  {
    ...
    Assert.assertTrue("sortedNodes was not empty but was expected to be.", sortedNodes.isEmpty());
  }
  ...
  public void testSortExecutableNodesSingleNodeFlow()
  {
    ...
    List<ExecutableNode> sortedNodes = ReportalUtil.sortExecutableNodes(executableFlow);
    Assert.assertEquals("sortedNodes was expected to have 1 node but had " + sortedNodes.size(), sortedNodes.size(), 1);
  }
  ...
  public void testSortExecutableNodesMultipleNodeFlow()
  {
    ...
    ExecutableFlow executableFlow = createExecutableFlow(nodes);
    List<ExecutableNode> sortedNodes = ReportalUtil.sortExecutableNodes(executableFlow);
    Assert.assertEquals("sortedNodes was expected to have " + SIZE + " nodes but had " + sortedNodes.size(), 
                        sortedNodes.size(), SIZE);
    
    for (int i = 0; i < SIZE; i++) {
      Assert.assertTrue("Node was expected to have an id of \"node" + i + "\" but instead had "
          + "an id of \"" + sortedNodes.get(i).getId() + "\"", sortedNodes.get(i).getId().equals("node" + i));
    }
  }
